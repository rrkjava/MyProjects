...
import my.mimos.misos.dissemination.domain.EmailConfig;
...
import my.mimos.misos.domain.integrationservice.EmailChannelResource;
...
//				}
				...
				// MAKING EMAIL RECIEPIENTS PARTITIONS
				int partitionSize = 99;
			    List<List<InternetAddress>> partitions = new ArrayList<>();
			    for (int i=0; i<addressList.size(); i += partitionSize) {
			        partitions.add(addressList.subList(i, Math.min(i + partitionSize, addressList.size())));
			    }

				// EMAIL SETTINGS
				InternetAddress senderAddress = new InternetAddress(emailConfig.getUsername().trim());
//				message.setRecipients(Message.RecipientType.BCC, address);
				message.setSender(senderAddress);
				helper.setFrom("misos@mimos.my");
				helper.setSubject(emailConfig.getSubjectMalay());
				...
				if (req.getMessageFormat().equalsIgnoreCase(MessageFormat.MF_TXT.toString())) {
					...
					messageBodyPart.setContent(formHTMLcontent(req.getEmailMessage()), "text/html");
...
//				javaMailSender.send(message);
				...
				// SENDING EACH PARTITION
				for (List<InternetAddress> list : partitions) {
			...
			res.setStatus("Unable to send email,Error from email service provider");
	 ...
	 * @return
	 */
	public List<String> resolveRecipients(EmailChannelRequestResource req) {

		List<String> emails = new ArrayList<String>();

		try {
			my.mimos.misos.domain.entity.trx.Message message = disseminationCommand
					.findByIocMessageId(req.getIocMessageId());

			RecipientRequestResource request = null;
			RecipientResponseResource recipients = null;
			List<EmailChannelResource> emailResource = null;
			...
			if (recipients.getStatus().getStatusCode().equals("200")) {
				...
				emailResource = disseminationChannel.getEmailChannel();
			...
			if (!emailResource.isEmpty()) {
				log.info("The integration service returned all users!");
				emailResource.forEach(recipient -> {
					if (validate(recipient.getEmail())) {
	...
	public ImageResource getImageResource(EmailChannelRequestResource req) {
		ImageResource image = null;
		ImageRequestResource imageRequestResource = null;
		List<String> messageList = new ArrayList<String>(Arrays.asList(req.getEmailMessage().split("<br>")));
		try {

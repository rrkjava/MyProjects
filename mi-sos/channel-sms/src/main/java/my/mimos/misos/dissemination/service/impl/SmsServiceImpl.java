...
import my.mimos.misos.dissemination.domain.SmsConfig;
...
import my.mimos.misos.domain.integrationservice.PhoneMobileResource;
...
public class SmsServiceImpl implements SmsService {
	...
	public SmsChannelResponseResource sendSms(SmsChannelRequestResource req) {
		...
		try {
			...
			String message = StringUtils.replace(req.getSmsMessage(), "<br>", "\n");// line breaks to newline
			...
			if (listAddr.size() > 0) {	
				...
				final String keyword = smsConfig.getKeyword();
				final String userName = smsConfig.getUsername();
				final String password = smsConfig.getPassword();
				...
				res.setStatus("SMS Dissemination success.");
	...
	public List<DestAddr> resolveRecipients(SmsChannelRequestResource req) {
		...
		try {
			...
			List<PhoneMobileResource> phoneMobileResource = null;
			...
			if (recipients.getStatus().getStatusCode().equals("200")) {
				...
				phoneMobileResource = disseminationChannel.getSmsChannel();
			...
			// 2. process response to get user sms numbers
			if (!phoneMobileResource.isEmpty()) {
				log.info("The integration service returned all users!");
				phoneMobileResource.forEach(recipient -> {
					listAddr.add(new DestAddr(recipient.getPhoneMobile()));
				});
			}
